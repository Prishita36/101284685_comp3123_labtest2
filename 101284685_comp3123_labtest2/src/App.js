      ...
      <div className="card">
        ...
        {loading ? (
          ...
          <div className="main-container">
            ...
            <h3>{weatherdata.weather[0].main}°C</h3>
            <div className="temperature">
             <h1>{parseFloat(weatherdata.main.temp - 273.15).toFixed(1)}</h1> 
            </div>
            <div className="location">
             <h3><i className="fa fa-street-view"></i>{weatherdata.name} | {weatherdata.sys.country}</h3>
            </div>
            <div className="temperature-range">
              <h6>Lat:{parseFloat(weatherdata.coords.lat).toFixed(1)}°C  
              || {parseFloat(weatherdata.coords.lon).toFixed(1)} °C || Humidity: {weatherdata.main.humidity}%</h6>
            </div>
         </div>
        ) : null}
          </>
